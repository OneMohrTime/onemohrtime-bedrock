...
export default async function compileScripts(esBuildOptions = null) {
    ...
    loconfig.tasks.scripts.forEach(async ({
        ...
        try {
          ...
          // errors management (already done in esbuild)
          if (err.errors && err.errors.length > 0) {
              const errorText = err.errors[0].text || 'Unknown error';
              const errorFile = err.errors[0].location?.file || 'Unknown file';
              const errorLine = err.errors[0].location?.line || 'Unknown line';

              notification({
                  title:   `${label} compilation failed 🚨`,
                  message: `${errorText} in ${errorFile} line ${errorLine}`,
              });
          } else {
